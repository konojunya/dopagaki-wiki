...
.steps{display:flex;gap:28px;align-items:stretch}.step{color:${color("step")};flex:1;min-width:0;border:${design.components["step-border"].width} solid ${color("step-border", "backgroundColor")};border-radius:${resolveReference(design, design.components.step.rounded)};padding:28px;display:flex;flex-direction:column;gap:${design.spacing.md};font-size:${theme.body}px;line-height:${type.body.lineHeight}}.step b{color:${theme.accent};font-size:${theme.body}px}.arrow{background:${color("flow-arrow", "backgroundColor")};align-self:center;font-size:52px;color:${color("flow-arrow")}}
.step-label{display:flex;gap:8px}.step-marker{flex:none;width:1em}
