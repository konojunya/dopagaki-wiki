...
export function buildTheme(design: Design) {
  ...
  const theme = {
    version: "accessible-light-v1.2.0",
...
.takeaway{color:${color("takeaway")};font-size:${theme.body}px;line-height:${type.body.lineHeight};padding:18px 24px;display:flex;align-items:flex-start;gap:${design.spacing.sm};border:${design.components.divider.height} solid ${color("divider", "backgroundColor")}}.takeaway-icon{flex:none}.takeaway>span:last-child{min-width:0}
...
.steps{display:flex;gap:28px;align-items:stretch}.step{color:${color("step")};flex:1;min-width:0;border:${design.components.divider.height} solid ${color("divider", "backgroundColor")};border-radius:${resolveReference(design, design.components.step.rounded)};padding:28px;display:flex;flex-direction:column;gap:${design.spacing.md};font-size:${theme.body}px;line-height:${type.body.lineHeight}}.step b{color:${theme.accent};font-size:${theme.body}px}.arrow{background:${color("flow-arrow", "backgroundColor")};align-self:center;font-size:52px;color:${color("flow-arrow")}}
.code-grid{display:grid;grid-template-columns:1.12fr 1fr;gap:44px;align-items:center}pre{margin:0;padding:${design.spacing.md};background:${color("code", "backgroundColor")};color:${color("code")};font:${type.code.fontWeight} ${type.code.fontSize}/${type.code.lineHeight} '${type.code.fontFamily}','Noto Sans JP',monospace;white-space:pre;tab-size:2} .explanations{margin:0;padding-left:1.5em;display:grid;gap:26px;list-style:decimal;font-size:${theme.body}px;line-height:${type.body.lineHeight}}.explanations li{padding-left:8px}.explanations li::marker{color:${theme.ink}}pre code{font:inherit}.hljs-keyword,.hljs-built_in,.hljs-type{color:${color("code-keyword")};font-weight:700}.hljs-number,.hljs-literal,.hljs-string,.hljs-title{color:${color("code-literal")}}.hljs-comment{color:${color("code-comment")}}
